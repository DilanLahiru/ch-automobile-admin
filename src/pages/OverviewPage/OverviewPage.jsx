...
const getUpcomingAppointments = (appointments) => {
  ...
  return appointments
    .filter((appointment) => {
      ...
      const isUpcoming =
        appointmentDate >= now && appointmentDate <= twoDaysLater;
      const isAcceptedOrPending = ["accepted", "pending"].includes(
        appointment.status,
      );
...
// Helper function to filter low stock items from products
const getLowStockItems = (products) => {
  if (!Array.isArray(products)) return [];

  return products
    .filter((product) => {
      // Check if stock is below minimum stock level
      const currentStock =
        product.quantity || product.stock || product.currentStock || 0;
      const minimumStock = product.minimumStock || 20;
    ...
    .map((product) => {
      ...
      return {
        ...
        partName: product.name || "Unknown",
        partNumber: product.sku || "N/A",
    ...
    .slice(0, 5); // Show top 5 lowest stock items
};

export function OverviewPage() {
  const [timeRange, setTimeRange] = useState("6M");
  const dispatch = useDispatch();
  const { appointments: appointmentsFromRedux, loading } = useSelector(
  ...
  useEffect(() => {
    if (appointmentsFromRedux && Array.isArray(appointmentsFromRedux)) {
      ...
      const pendingAppointments = appointmentsFromRedux.filter(
        (app) => app.status === "pending",
      ).length;
      const acceptedAppointments = appointmentsFromRedux.filter(
        (app) => app.status === "accepted",
      ).length;
      const rejectedAppointments = appointmentsFromRedux.filter(
        (app) => app.status === "rejected",
      ).length;
      const cancelledAppointments = appointmentsFromRedux.filter(
        (app) => app.status === "cancelled",
      ).length;
      const completedAppointments = appointmentsFromRedux.filter(
      ...
      const customersChange =
        uniqueCustomers > 0 ? `+${uniqueCustomers} total` : "0 total";
      ...
      const chartData = Object.keys(serviceTypeCounts).map(
        (serviceType, index) => ({
          name: serviceType,
  ...
  // Handle products data for low stock alert
  useEffect(() => {
    if (products && Array.isArray(products)) {
      const lowStockData = getLowStockItems(products);
        ...
        {summaryStats.map((stat, index) => (
          ...
          >
            <div
              ...
              style={{
                background: `linear-gradient(135deg, ${stat.chartColor}20, transparent)`,
              }}
            ></div>
            <div className="relative bg-white rounded-2xl border shadow-md border-gray-200 p-6 hover:shadow-xl transition-shadow">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-gray-500">
              Scheduled service appointments for the next 48 hours
            </p>
          </div>
          <Button
            variant="outline"
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div
                        className={`w-3 h-3 rounded-full ${getUrgencyColor(item.urgency)}`}
                      ></div>
                      <div>
                        <div className="flex items-center gap-2">
          ...
          {topCustomers.length > 0 ? (
            ...
            <div className="text-center py-8">
              ...
              <p className="text-sm text-gray-500 mt-2">
                Customer information will appear after appointments are created
              </p>
            </div>
          )}
        </Card>
